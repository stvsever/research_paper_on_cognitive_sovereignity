...
\section{Results}
Run 6 generated 100 pseudoprofiles and 400 attacked opinion rows. The fixed attack leaf was linked to 4 repeated opinion indicators per profile. Mean absolute attacked shift was 43.35 and mean signed shift was 22.86. Mean attack realism was 0.71, while mean post-exposure plausibility was 0.73.

The measurement-and-moderation structure is summarized in Figure~\ref{fig:design}. Figure~\ref{fig:absdelta} shows the distribution of absolute attacked deltas across repeated opinion leaves. Figure~\ref{fig:forest} shows the descriptive susceptibility weights that summarize the post hoc profile index, and Figure~\ref{fig:sem} shows the repeated-outcome path-SEM coefficient matrix. The strongest normalized moderator weights in run 6 were Sex Male (8.5\% of normalized moderator weight); Age Years (8.5\% of normalized moderator weight); Sex Female (6.9\% of normalized moderator weight); Big Five Neuroticism Impulsiveness \% (6.0\% of normalized moderator weight). At the ontology-group level, the largest aggregated weight shares were Personality: Extraversion (20.6\%); Demographics: Sex (18.2\%); Personality: Openness (15.7\%).
...
\label{fig:forest}
\caption*{\raggedright \footnotesize Note. Bars show the normalized contribution of each profile moderator to the post hoc susceptibility index under the configured attack/opinion target set.}
\end{figure}
...
The repeated-outcome path SEM converged with CFI = 1.000 and RMSEA = 0.000. Those fit indices should still be read cautiously because the model is relatively dense for a pilot of this size and several paths are being screened simultaneously. The clearest leaf-specific paths were Sex Female -> Adversarial Delta Indicator  Alliance Commitment Support (b = 30.88, p = 0.001); Big Five Neuroticism Mean \% -> Adversarial Delta Indicator  Civil Defense Preparedness Support (b = 8.23, p = 0.051); Sex Female -> Adversarial Delta Indicator  Civil Defense Preparedness Support (b = -16.90, p = 0.089); Sex Other -> Adversarial Delta Indicator  Strategic Autonomy Support (b = -15.17, p = 0.143); Sex Other -> Adversarial Delta Indicator  Alliance Commitment Support (b = 12.83, p = 0.211); Sex Other -> Adversarial Delta Indicator  Civil Defense Preparedness Support (b = -12.65, p = 0.256). These p-values are unadjusted and should be interpreted as exploratory. The post hoc susceptibility ranking identified the highest-scoring profiles as profile\_0344 (100.0th percentile; mean |Δ| = 26.2); profile\_0036 (99.0th percentile; mean |Δ| = 53.2); profile\_0288 (98.0th percentile; mean |Δ| = 42.5). The Spearman alignment between the post hoc susceptibility ranking and the observed effectivity percentile was 0.149, which is useful descriptively but should not be over-interpreted as external validation. Controlled summary contrasts and ridge-derived weights together suggested that the most influential profile components in this pilot were Sex Female (controlled b = 5.64, p = 0.110, weight = 6.9\%); Sex Other (controlled b = -7.22, p = 0.064, weight = 2.8\%); Big Five Neuroticism Mean \% (controlled b = 2.01, p = 0.214, weight = n/a\%); Big Five Conscientiousness Mean \% (controlled b = -1.93, p = 0.280, weight = n/a\%); Age Years (controlled b = 0.08, p = 0.968, weight = n/a\%).
...
\label{fig:s3}
\caption*{\raggedright \footnotesize Note. The susceptibility index is the percentile-ranked profile-only linear predictor from the target-conditional ridge aggregation of fitted attack-opinion task models. It is descriptive rather than independent evidence.}
\end{figure}
